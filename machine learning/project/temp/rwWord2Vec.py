...
def word2vecClassifier(X,y):
  ...
  preprocessedX=PreprocessSamples(X)
  sparseX = vectorizer.fit_transform(preprocessedX)

  #generate dense word2vec matrix
  sentences = []
  for entry in X:
    sentence=[]
  ...
  #combine dense/sparse matrices
  combination=hstack([vectorized, sparseX])
  #print combination[0]
  ...
  print "word2Vec"
  MultinomialNaiveBayes(combination, y)
  DecisionTreeStump(combination, y)
  AdaBoostDecisionTree(combination, y)
  PerceptronPrediction(combination, y)
  #AdaboostPerceptron(combination, y)
  print "\n"
