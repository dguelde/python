...
def MaxStrength(Q,vacnum):
    ...
    for s in sample:
        samplestrength.append(Q.degree(s,'weight'))
    sarray = np.array(sample)
    sdarray = np.array(samplestrength)
...
vaccinated = []
for vacnum in range(1,392,1):
    vaccinated = MaxStrength(N,vacnum*2)
    test = sucept.copy()
...
np.savetxt('MaxStrengthvaclen.txt',timesteparr,fmt='%4.5e')
np.savetxt('MaxStrengthvacsize.txt',sizearr,fmt='%4.5e')
"""
